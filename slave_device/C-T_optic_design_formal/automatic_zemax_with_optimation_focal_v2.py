...
class SpectrometerInputs:
    """Inputs to the CT spectrometer design calculator.

    Notes
    -----
    * grating_lines_per_mm: 光栅刻线密度（lines/mm），对应你公式中的 G 或 f（注意不要与焦距混淆）。
    * f_coll_mm / f_cam_mm: 选用的商用凹面镜标称焦距（例如 Thorlabs CM254-050-G01: 50 mm；
      CM254-075-M01: 75 mm）。默认只用于在 MFE 中对曲率施加约束，使优化结果贴近可采购器件。
    """
    ...
    # 商用凹面镜标称焦距（mm）
    f_coll_mm: float = 50.0  # 准直镜
    f_cam_mm: float = 75.0   # 汇聚镜
...
class ComputedParams:
    ...
    # mirror nominal focal lengths (mm)
    f_coll_mm: float
    f_cam_mm: float
...
class CTDesignCalculator:
    ...
    def compute(inputs: SpectrometerInputs) -> ComputedParams:
        ...
        F_1 = float(inputs.f_coll_mm)
        F_2 = float(inputs.f_cam_mm)
        ...
        R_1 = _calc.compute_R_1_known_f1_f2(F_1)
        R_2 = _calc.compute_R_2_known_f1_f2(F_2)

        theta_2_rad = _calc.compute_theta_2(theta_1_rad, alpha_rad, beta_rad, M)

        L_out = _calc.compute_L_out_known_f1_f2(theta_2_rad, R_2)
        L_in = _calc.compute_L_in_known_f1_f2(theta_1_rad, R_1)
...
class MeritConstraintsConfig:
    ...
    # curvature bounds (1/mm) for mirrors (LDE radii are negative)
    R_1_min: float = 0.0
    R_1_max: float = 0.0
    R_2_min: float = 0.0
    R_2_max: float = 0.0
...
def push_to_zemax(
    *,
    ...
    sinputs = SpectrometerInputs(
        ...
        f_coll_mm=float(inputs["F_1"]),
        f_cam_mm=float(inputs["F_2"]),
    )
...
def info2(p):
    ...
    print("准直镜标称焦距 F1 (mm):", p.f_coll_mm)
    print("汇聚镜标称焦距 F2 (mm):", p.f_cam_mm)
...
def calculate_micro_adjust(opts, p):
    ...
    R1_t = 2.0 * float(getattr(p, "f_coll_mm", 50.0))  # nominal RoC for collimator mirror
    R2_t = 2.0 * float(getattr(p, "f_cam_mm", 75.0))   # nominal RoC for camera mirror

    opts.constraints_cfg.R_1_min = -1.0 / (R1_t - dR)
    opts.constraints_cfg.R_1_max = -1.0 / (R1_t + dR)
    opts.constraints_cfg.R_2_min = -1.0 / (R2_t - dR)
    opts.constraints_cfg.R_2_max = -1.0 / (R2_t + dR)
    return opts
...
if __name__ == "__main__":
    ...
    other_settings = {
        ...
        "NA": 0.125, # 入射光的参数
        "save_path": None,
        "auto_set_variables_pickups": False, # 参数设置为变量，为了后续优化
