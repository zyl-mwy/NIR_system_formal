...
class MeritFunctionBuilder:
    ...
    def configure_cross_ct(
        ...
        *,
        ...
        if "surf_silt" in basicStructure:
            thickness_list[thickness_list.index(("FTGT", basicStructure.index("surf_object"), cfg["Lin_min_mm"]))] = ("FTGT", basicStructure.index("surf_silt"), cfg["Lin_min_mm"])
            thickness_list[thickness_list.index(("FTLT", basicStructure.index("surf_object"), cfg["Lin_max_mm"]))] = ("FTLT", basicStructure.index("surf_silt"), cfg["Lin_max_mm"])
        # 10-17) thickness bounds
        for op_type, surf, tgt in thickness_list:
...
class SolveConfigurator:
    ...
    def configure_cross_ct(self, system: Any, ZOSAPI: Any, opts: Dict[str, Any], basicStructure: list) -> None:
        ...
        if "surf_silt" in basicStructure:
            index_object = variable_cells.index([basicStructure.index("surf_object"), SC.Thickness])
            variable_cells[index_object] = [basicStructure.index("surf_silt"), SC.Thickness]
...
class LDEBuilder:
    ...
    def build_cross_ct_like_base0(self, system: Any, ZOSAPI: Any, p: Dict[str, Any], opts: Dict[str, Any], basicStructure: list) -> None:
        ...
        if other_settings["need_parallel_light"] == 0:
        ...
        if "surf_silt" in basicStructure:
            ...
            # s0_0 = lde.GetSurfaceAt(basicStructure.index("surf_silt_0"))
            # s0_0.Thickness = 2.5
            # s0_0.Comment = "模拟狭缝实际大小"
            # s0_0.SemiDiameter = 12.7


            s0_1 = lde.GetSurfaceAt(basicStructure.index("surf_silt"))
            s0_1.Thickness = float(p["L_in"])
            s0_1.Comment = "模拟狭缝外围尺寸"
            s0_1.SemiDiameter = 12.7

            # s0_1.DrawData.MirrorSubstrate = ZOSAPI.Editors.LDE.SubstrateType.Flat
            # s0_1.DrawData.MirrorThickness = float(2.5)


            # ap = s0_1.ApertureData.CreateApertureTypeSettings(
            #     ZOSAPI.Editors.LDE.SurfaceApertureTypes.CircularAperture
            # )
            # ap.MaximumRadius = 12.7
            # s0_1.ApertureData.ChangeApertureTypeSettings(ap)

        # Surface 1: coordinate break; tilt x = theta_1

        # "need_focus": [
        #     {"comment": "AC127-025-C_0", "f_mm": 12.0, "d_mm": 4.7, "material": "N-LAK22", "surface":"THORC"},
        #     {"comment": "AC127-025-C_1", "f_mm": -12.940, "d_mm": 1.5, "material": "N-SF6HT", "surface":""},
        #     {"comment": "AC127-025-C_2", "f_mm": 151.710, "d_mm": 20.197, "material": "", "surface":"THORCSLAH64"},
        # ]

        if "surf_aperture" in basicStructure:
            s0_9 = lde.GetSurfaceAt(basicStructure.index("surf_aperture"))
            s0_9.Thickness = float(10.0)
            s0_9.Comment = "光阑"
            s0_9.SemiDiameter = other_settings["aperture"]/2.0

            aper = s0_9.ApertureData
            # 创建圆形 Surface Aperture 设置
            circ = aper.CreateApertureTypeSettings(
                ZOSAPI.Editors.LDE.SurfaceApertureTypes.CircularAperture
            )

            # 关键：对底层实现对象赋值，并用它提交
            circ_impl = getattr(circ, "__implementation__", circ)
            circ_impl.MinimumRadius = 0.0
            circ_impl.MaximumRadius = float(other_settings["aperture"]) / 2.0
            aper.ChangeApertureTypeSettings(circ_impl)

            # 强制刷新一次（可选但推荐）
            # system.UpdateStatus()

            # 回读验证
            # cur = getattr(aper.CurrentTypeSettings, "__implementation__", aper.CurrentTypeSettings)
            # print("Aperture MaxR =", cur.MaximumRadius)
        

        if "surf_silt_real_size" in basicStructure:
            # print("---------------------set slit real size-----------------")
            s0_8 = lde.GetSurfaceAt(basicStructure.index("surf_silt_real_size"))
            s0_8.Comment = "模拟狭缝长宽"
            s0_8.SemiDiameter = other_settings["slit_real_size_mm"][0]/2.0

            ap = s0_8.ApertureData.CreateApertureTypeSettings(
                ZOSAPI.Editors.LDE.SurfaceApertureTypes.RectangularAperture
            )._S_RectangularAperture
            ap.XHalfWidth = other_settings["slit_real_size_mm"][0]/2.0
            ap.YHalfWidth = other_settings["slit_real_size_mm"][1]/2.0
            ok = s0_8.ApertureData.ChangeApertureTypeSettings(ap)

        if len(other_settings["need_focus"]) != 0:
            s0_2 = lde.GetSurfaceAt(basicStructure.index(other_settings["need_focus"][0]["comment"]))
            s0_2.Comment = other_settings["need_focus"][0]["comment"]
            s0_2.Radius = float(other_settings["need_focus"][0]["f_mm"])
            s0_2.Thickness = float(other_settings["need_focus"][0]["d_mm"])
            s0_2.Material = other_settings["need_focus"][0]["material"]
            s0_2.Coating = other_settings["need_focus"][0]["surface"]

            s0_3 = lde.GetSurfaceAt(basicStructure.index(other_settings["need_focus"][1]["comment"]))
            s0_3.Comment = other_settings["need_focus"][1]["comment"]
            s0_3.Radius = float(other_settings["need_focus"][1]["f_mm"])
            s0_3.Thickness = float(other_settings["need_focus"][1]["d_mm"])
            s0_3.Material = other_settings["need_focus"][1]["material"]
            s0_3.Coating = other_settings["need_focus"][1]["surface"]

            s0_4 = lde.GetSurfaceAt(basicStructure.index(other_settings["need_focus"][2]["comment"]))
            s0_4.Comment = other_settings["need_focus"][2]["comment"]
            s0_4.Radius = float(other_settings["need_focus"][2]["f_mm"])
            s0_4.Thickness = float(other_settings["need_focus"][2]["d_mm"])
            s0_4.Material = other_settings["need_focus"][2]["material"]
            s0_4.Coating = other_settings["need_focus"][2]["surface"]

            if opts["real_size"] == True:
            # set size
                s0_2.SemiDiameter = other_settings["need_focus"][0]["SemiDiameter"]
                s0_3.SemiDiameter = other_settings["need_focus"][1]["SemiDiameter"]
                s0_4.SemiDiameter = other_settings["need_focus"][2]["SemiDiameter"]
        # need_parallel_mirror
        if len(other_settings["need_parallel_mirror"]) != 0:
            s0_5 = lde.GetSurfaceAt(basicStructure.index(other_settings["need_parallel_mirror"][0]["comment"]))
            s0_5.Comment = other_settings["need_parallel_mirror"][0]["comment"]
            # s0_5.Radius = float(other_settings["need_parallel_mirror"][0]["f_mm"])
            s0_5.Thickness = float(other_settings["need_parallel_mirror"][0]["d_mm"])
            s0_5.Material = other_settings["need_parallel_mirror"][0]["material"]
            s0_5.Coating = other_settings["need_parallel_mirror"][0]["surface"]

            s0_6 = lde.GetSurfaceAt(basicStructure.index(other_settings["need_parallel_mirror"][1]["comment"]))
            s0_6.Comment = other_settings["need_parallel_mirror"][1]["comment"]
            s0_6.Radius = float(other_settings["need_parallel_mirror"][1]["f_mm"])
            s0_6.Thickness = float(other_settings["need_parallel_mirror"][1]["d_mm"])
            s0_6.Material = other_settings["need_parallel_mirror"][1]["material"]
            s0_6.Coating = other_settings["need_parallel_mirror"][1]["surface"]

            s0_7 = lde.GetSurfaceAt(basicStructure.index(other_settings["need_parallel_mirror"][2]["comment"]))
            s0_7.Comment = other_settings["need_parallel_mirror"][2]["comment"]
            s0_7.Radius = float(other_settings["need_parallel_mirror"][2]["f_mm"])
            s0_7.Thickness = float(other_settings["need_parallel_mirror"][2]["d_mm"])
            s0_7.Material = other_settings["need_parallel_mirror"][2]["material"]
            s0_7.Coating = other_settings["need_parallel_mirror"][2]["surface"]

            s_type = s0_7.GetSurfaceTypeSettings(ZOSAPI.Editors.LDE.SurfaceType.EvenAspheric)
            s0_7.ChangeType(s_type)

            s0_7.Conic = float(other_settings["need_parallel_mirror"][2]["Conic"])

            s0_7.GetSurfaceCell(ZOSAPI.Editors.LDE.SurfaceColumn.Par2).DoubleValue = float(other_settings["need_parallel_mirror"][2]["a4"])
            s0_7.GetSurfaceCell(ZOSAPI.Editors.LDE.SurfaceColumn.Par3).DoubleValue = float(other_settings["need_parallel_mirror"][2]["a6"])

            if opts["real_size"] == True:
            # set size
                # s0_5.SemiDiameter = other_settings["need_parallel_mirror"][0]["SemiDiameter"]
                s0_6.SemiDiameter = other_settings["need_parallel_mirror"][1]["SemiDiameter"]
                s0_7.SemiDiameter = other_settings["need_parallel_mirror"][2]["SemiDiameter"]


        if other_settings["parallel_length_mm"] != 0:
            s0_5 = lde.GetSurfaceAt(basicStructure.index("surf_parallel"))
            s0_5.Thickness = float(other_settings["parallel_length_mm"])
            s0_5.Comment = "模拟准直器长度"
...
class ZemaxHandle:
    ...
    def configure_light(
        ...
        *,
        ...
        if need_parallel_light == 0:
            ...
            # Aperture: Entrance Pupil Diameter (EPD)
            sd.Aperture.ApertureType = ZOSAPI.SystemData.ZemaxApertureType.EntrancePupilDiameter
            sd.Aperture.ApertureValue = float(need_parallel_light)  # 这里是“直径”，单位=Lens Units(通常mm)
    ...
    def configure_basic_Structure(
        ...
        if other_settings["need_silt"] == True:
            basicStructure.insert(1, "surf_silt")

        if len(other_settings["slit_real_size_mm"]) != 0:
            index_surf_silt = basicStructure.index("surf_silt")
            basicStructure.insert(index_surf_silt, "surf_silt_real_size")
        ...
        if len(other_settings["need_focus"]) != 0:
            index_surf_object = basicStructure.index("surf_object")
            for i in range(len(other_settings["need_focus"])-1, -1, -1):
                basicStructure.insert(index_surf_object+1, other_settings["need_focus"][i]["comment"])
        if other_settings["parallel_length_mm"] != 0:
            index_surf_object = basicStructure.index("surf_object")
            basicStructure.insert(index_surf_object+1, "surf_parallel")

        if other_settings["aperture"] != 0:
            index_surf_object = basicStructure.index("surf_object")
            basicStructure.insert(index_surf_object+1, "surf_aperture")


        if len(other_settings["need_parallel_mirror"]) != 0:
            index_surf_object = basicStructure.index("surf_object")
            for i in range(len(other_settings["need_parallel_mirror"])-1, -1, -1):
                basicStructure.insert(index_surf_object+1, other_settings["need_parallel_mirror"][i]["comment"])
    ...
    def push_to_zemax(
        ...
        self.configure_light(
                ...
                fields_x=(-other_settings["fiber_diameter"]/2, 0.0, other_settings["fiber_diameter"]/2),
                wavelengths_um=(basic_parameter["lambda_1"] / 1000.0, basic_parameter["lambda_c"] / 1000.0, basic_parameter["lambda_2"] / 1000.0),
                need_parallel_light=other_settings["need_parallel_light"],
            )
...
class ComputeForZemax:
    ...
    def compute_basic_parameter(self, inputs: Dict[str, Any], other_settings: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.d_2 = self.compute_d_1(self.R_1)
...
if __name__ == "__main__":
    ...
    other_settings = {
        ...
        "need_silt": True,
        "slit_real_size_mm": [3, 0.01], # mm
        "need_parallel_light": 0, # 4
        "need_focus": [
            # {"comment": "双胶合透镜AC127-025-C_0", "f_mm": 1.200000000000001E+001, "d_mm": 4.700000000000000E+000, "material": "N-LAK22", "surface":"THORC", "SemiDiameter": 6.35},
            # {"comment": "双胶合透镜AC127-025-C_1", "f_mm": -1.294000000000001E+001, "d_mm": 1.500000000000000E+000, "material": "N-SF6HT", "surface":"", "SemiDiameter": 6.35},
            # # {"comment": "AC127-025-C_2", "f_mm": 151.710, "d_mm": 20.197, "material": "", "surface":"THORCSLAH64", "SemiDiameter": 6.35},
            # {"comment": "双胶合透镜AC127-025-C_2", "f_mm": 1.517099999999993E+002, "d_mm": 2.019711878358000E+001, "material": "", "surface":"THORCSLAH64", "SemiDiameter": 6.35},
        
            {"comment": "双胶合透镜AC127-025-C_0", "f_mm": 1.225416791872433E+001, "d_mm": 2.250004129282395E+000, "material": "N-PK52A", "surface":"THORC", "SemiDiameter": 4.000000000000000E+000},
            {"comment": "双胶合透镜AC127-025-C_1", "f_mm": -1.603394126687054E+001, "d_mm": 2.250008926927114E+000, "material": "N-SF6", "surface":"", "SemiDiameter": 4.000000000000000E+000},
            # {"comment": "AC127-025-C_2", "f_mm": 151.710, "d_mm": 20.197, "material": "", "surface":"THORCSLAH64", "SemiDiameter": 6.35},
            {"comment": "双胶合透镜AC127-025-C_2", "f_mm": -7.041013851415100E+001, "d_mm": 2.729954975880598E+001, "material": "", "surface":"THORC", "SemiDiameter": 4.000000000000000E+000},
        
        ],
        "parallel_length_mm": 0, # 10,
        "need_parallel_mirror": [
            {"comment": "准直镜F220SMA-1310_0", "f_mm": 0, "d_mm": 8.054245951471000E+000, "material": "", "surface":"", "SemiDiameter": 0},
            {"comment": "准直镜F220SMA-1310_1", "f_mm": 0, "d_mm": 5.031595527853000E+000, "material": "D-ZK3M", "surface":"THORA", "SemiDiameter": 2.75},
            {"comment": "准直镜F220SMA-1310_2", "f_mm": -6.428132050907563E+000, "d_mm": 1.125697110476000E+001, "material": "", "surface":"THORA", "SemiDiameter": 2.75, "Conic": -7.312835995110000E-001, "a4": -8.924167336704999E-005, "a6": -4.384364140114000E-007},
        ],
        "fiber_diameter": 0.05, # mm
        "aperture": 3.0, # mm
    }
    ...
    basicStructure = [
        ...
        # basicStructure.insert(1, "surf_silt_0")
