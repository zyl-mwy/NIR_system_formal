 ...
 * 
 * 编译方法:
 *   g++ -std=c++17 onnx_example.cpp -lonnxruntime -o onnx_example
 * 
 * 或者使用 CMake (推荐):
...
public:
    ...
    float predict(const std::vector<float>& spectrum_data) {
        ...
        // 准备输入数据
        std::vector<int64_t> input_shape = {1, 1024};  // [batch_size, 1024]
    ...
    std::vector<float> predictBatch(const std::vector<std::vector<float>>& spectra) {
        ...
        std::vector<int64_t> input_shape = {static_cast<int64_t>(batch_size), 1024};
...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        ...
        std::cerr << "示例: " << argv[0] << " spectrum_model.onnx" << std::endl;
        return 1;
    }
