...
class SpectrumPredictor:
    ...
    def __init__(self, model_type='svm'):
        ...
        Args:
            model_type: 模型类型，可选 'random_forest', 'svm', 'linear'
            注意：此文件夹默认使用支持向量机（SVR）算法
        """
    ...
    def train(self, X: np.ndarray, y: np.ndarray, test_size: float = 0.2, 
        ...
        # 创建并训练模型（核心算法：支持向量机 SVR）
        if self.model_type == 'svm':
            # 支持向量机回归器
            C = kwargs.get('C', 1.0)  # 正则化参数
            epsilon = kwargs.get('epsilon', 0.1)  # 不敏感区域的宽度
            kernel = kwargs.get('kernel', 'rbf')  # 核函数类型：'rbf', 'linear', 'poly'
            gamma = kwargs.get('gamma', 'scale')  # RBF 核的 gamma 参数
...
def example_basic_usage():
    ...
    # 创建预测器（使用支持向量机算法）
    predictor = SpectrumPredictor(model_type='svm')
    ...
    # 训练模型（使用支持向量机 SVR）
    print("\n训练模型（使用支持向量机 SVR）...")
    results = predictor.train(X, y, test_size=0.2, C=10.0, epsilon=0.1, kernel='rbf')
...
def example_save_and_load():
    ...
    predictor.train(X, y, C=10.0, epsilon=0.1, kernel='rbf')
...
def example_custom_data():
    ...
    # 训练模型（使用支持向量机算法）
    predictor = SpectrumPredictor(model_type='svm')
    predictor.train(custom_X, custom_y, C=10.0, epsilon=0.1, kernel='rbf')
